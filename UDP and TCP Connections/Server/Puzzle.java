...
public class Puzzle {
    ...
    /************************** NEEDS TO BE CHANGED FOR SERVER IMPLEMENTATION ********************
     * Name: requestWord
    ...
    private String requestWord(int minLength, char firstLetter, char lastLetter, Scanner scanner) {
        ...
        SendUDPRequest send = new SendUDPRequest(WORD_REPO);
        send.sendUDPRequest(message);

        String response = send.getResponse();
        String[] parts = response.split(" ");
        if (parts.length != 2 || !parts[0].equals("rw")) {
            return "Invalid Word";
        }
